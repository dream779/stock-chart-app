...
如果你所在的网络无法访问新浪财经或天天基金（东方财富），可以开启 Mock 数据模式：
...
│   ├── eastmoney.ts                       # 新浪盘中估值 + 天天基金确认净值/历史净值
│   ├── holdings.ts                        # 持仓领域逻辑
...
- **行情数据**（`/api/fund/:code`）：不缓存，每次页面加载即时请求
- **历史数据**（`/api/fund/historical/:code`）：内存缓存 1 小时
- 历史缓存只在 Serverless 实例存活期间有效，实例冷启动后会重新获取
...
本项目仅用于学习和个人参考，盘中估值来源于新浪财经，最新确认净值与历史净值来源于天天基金（东方财富），数据可能存在延迟或估算偏差。数据仅供参考，不构成任何投资建议。
